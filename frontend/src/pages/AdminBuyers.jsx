...
export default function AdminBuyers() {
  const [buyers, setBuyers] = useState([
    { id: 1, name: "Example Corp", email: "buyer@example.com", status: "active", totalCredits: 120 },
    { id: 2, name: "GreenTech Ltd.", email: "gt@example.com", status: "suspended", totalCredits: 450 },
  ]);
        ...
        {buyers.map(b => (
          <div key={b.id} className="bg-white p-4 rounded-lg shadow flex justify-between items-center">
            <div>
              ...
              <p className="text-sm text-gray-600">{b.email} • Credits: {b.totalCredits}</p>
            </div>

            <div className="flex gap-3">
              <span className={`px-2 py-1 rounded text-sm ${b.status === 'active' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                {b.status}
              </span>
              <button onClick={() => toggleBlock(b.id)} className="px-3 py-1 bg-red-600 text-white rounded">
