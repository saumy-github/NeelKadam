...
export default function AdminDashboard() {
  ...
  const fetchDashboardStats = async () => {
    try {
      ...
      if (response.stats) {
        setStats({
          totalSellers: response.stats.ngos.total || 0,
          totalBuyers: response.stats.buyers.total || 0,
          totalProjects: response.stats.projects.total || 0,
        });
      }
    } catch (error) {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gradient-to-br from-slate-50 via-blue-50 to-slate-50">
      ...
      <main className="flex-grow px-10 py-12 max-w-7xl mx-auto space-y-14 overflow-y-auto">
        <h2 className="text-3xl font-black text-red-700 drop-shadow-md">
          Welcome, Admin
        </h2>
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {[
            ...
            { label: "Total Buyers", value: stats.totalBuyers, color: "blue" },
            {
            ...
            >
              <h3 className={`text-2xl font-bold text-${stat.color}-700 mb-4`}>
                {stat.label}
              </h3>
